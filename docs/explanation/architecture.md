...
## Why Tailwind's standalone CLI, not Node or a CDN script

Styling `internal/notes` needed a real CSS pipeline (purging, `@theme`
tokens, arbitrary-value resolution) — not the "write more hand-rolled
CSS" non-choice, and not a CDN `<script>` tag either, since that's a
runtime fetch that directly violates AD-9's "one self-contained binary,
no separate thing that has to travel with it" goal. Tailwind v4's
standalone CLI is a single per-platform binary with no Node.js/npm
dependency, fitting the same build-time-tool category this template
already uses for `templ`/`sqlc`/`just` — pinned to an exact version,
checksum-verified before use, documented in `AGENTS.md`'s Toolchain
section. Its *output* (`app.css`) is treated exactly like `templ`'s and
`sqlc`'s generated Go code: gitignored, regenerated by `just generate`,
never a source of drift because the source of truth (`input.css`) is
what's committed. See [Style with Tailwind](../how-to/style-with-tailwind.md).

## Why a proportionate security-headers pass, not a general-purpose kit

`internal/notes` is deliberately minimal: an anonymous, public example
app with no user accounts or sessions. That specific threat model is
what decides what its hardening pass covers — a small `SecurityHeaders`
middleware (`X-Content-Type-Options`, `Referrer-Policy`,
`X-Frame-Options`, a same-origin `Content-Security-Policy`) and a
request-body size cap on the one endpoint that accepts user input, not
CSRF tokens or rate limiting. CSRF protection defends a session an
attacker could ride on; with no session to forge, adding token
machinery would be security theater dressed up as diligence, not a
real control. If a real consumer project adds authentication, CSRF
protection becomes a genuine requirement at that point — see
[Escalating beyond the defaults](../how-to/escalate-beyond-the-defaults.md).

A strict CSP is not a drop-in change over an existing UI without
checking it: this template's own CSP rollout broke two things that
worked fine without it — `hx-on::after:request="this.reset()"`, which
evaluates its argument via `new AsyncFunction` (real eval, blocked
without `unsafe-eval`), and an inline `style="display:none"` attribute
